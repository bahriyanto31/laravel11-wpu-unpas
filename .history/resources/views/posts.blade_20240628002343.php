<x-layout>
    <x-slot:title>{{ $title }} <span class="text-sm float-end text-gray-500">{{ count($posts) }}
            article</span></x-slot:title>
    ...
    <div class="py-4 px-4 mx-auto max-w-screen-xl lg:py-4 lg:px-0">
        <div class="grid gap-8 {{ count($posts) > 0 ? 'lg:grid-cols-3 md:grid-cols-2' : '' }}">
            @forelse ($posts as $post)
                <article
                    class="p-6 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                    <div class="flex justify-between items-center mb-5 text-gray-500">
                        <a href="/blogs?kategori={{ $post->kategori->slug }}"
            ...
            @empty
                <div class="text-center"><a href="/blogs"
                        class="font-medium text-xs text-blue-500 hover:underline">&laquo; Back to all
                        posts</a>
